...
This code example demonstrates the System Control Unit (SCU) sleep mode on the [XMC™ MCU evaluation kits](#supported-kits-make-variable-target).
...
3. Confirm the operation of the example:
   ...
   1. If using the XMC1400 Boot Kit, confirm that the User LED glows when the knob of the potentiometer(on P2.5) is turned backward and forward. Confirm that the User LED goes off after a cyclic delay during the interrupt service routine and MCU goes to sleep again.

      See the [XMC1400 Boot kit Board User Manual](https://www.infineon.com/dgdl/Infineon-Board_Users_Manual_XMC1400_Boot_Kit.pdf-UM-v01_00-EN.pdf?fileId=5546d462525dbac401527815f9a073fd) for details of the potentiometer connections.

   2. If using the XMC4700 Relax Kit, connect an external jumper wire at P1.15 and 3V3 of connector X2 on the evaluation kit. Confirm that the User LED glows when a connection is made between 3V3 and P1.15 port pin. Confirm that the User LED goes off after a cyclic delay during the interrupt service routine and MCU goes to sleep again.

      See the [XMC4700 Relax kit Board User Manual](https://www.infineon.com/dgdl/Infineon-Board_User_Manual_XMC4700_XMC4800_Relax_Kit_Series-UM-v01_02-EN.pdf?fileId=5546d46250cc1fdf01513f8e052d07fc) for details of  the Port pins.
...
P2.5 is configured as an input port for the XMC1400 Boot Kit by the Device Configurator as shown below:

**Figure 1. P2.5 Configuration**

![](images/xmc_port.png)

The default port configuration is used for the XMC4700 Relax Kit.

XMC1400 uses ERU group0 with the event trigger logic on channel 1 (`ERU0_ETL1`), while XMC4700 uses ERU group1 with the event trigger logic on channel 1 (`ERU1_ETL1`). The output gating unit (`ERU_OGU`) is configured for service requests on the external trigger input.

The main loop calls the `_WFI()` function to put the controller into sleep mode. A falling edge on the external signal will wake up the MCU; the LED glows during the ERU interrupt service routine and then goes off after a cyclic delay.

The CPU is put back to sleep mode again after the interrupt service routine and calling the `_WFI()` function.
